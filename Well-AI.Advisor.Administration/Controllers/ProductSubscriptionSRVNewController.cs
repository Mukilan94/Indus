...
namespace Well_AI.Advisor.Administration.Controllers
{
   // [Area("ServiceCompany")]
    public class ProductSubscriptionSRVNewController : BaseController
    {
        ...
        ////  private readonly UserManager<WellIdentityUser> _userManager;
        //public ProductSubscriptionSRVNewController(IConfiguration _configuration, UserManager<WellIdentityUser> userManager,
        ...
        public async Task<IActionResult> Index( string id)
        {
            ...
            try
            {
                ...
                /// var tenantId = WellAIAppContext.Current.Session.GetString("TenantId_Admin");
                var tenantId = id;
                WellAIAppContext.Current.Session.SetString("TenantId_Admin", tenantId);
                //Customer details
                ...
                ViewBag.SelectedAccountType = db.Subscription.Where(s => s.TenantId == tenantId).Select(s => s.SubscriptionType).FirstOrDefault();
            ...
            {
                CustomErrorHandler customErrorHandler = new CustomErrorHandler(_roleManager, _userManager, db, Guid.Parse(_userManager.GetUserId(User)), Guid.Parse(WellAIAppContext.Current.Session.GetString("TenantId_Admin")));
        ...
        public IActionResult ChangepaymentAD()
        {
            try
            {
                //if (_signInManager.IsSignedIn(User) == false)
                ...
                //   return View();
                return PartialView("ChangepaymentAD");
            }
            catch (Exception ex)
            {
                CustomErrorHandler customErrorHandler = new CustomErrorHandler(_roleManager, _userManager, db, Guid.Parse(_userManager.GetUserId(User)), Guid.Parse(WellAIAppContext.Current.Session.GetString("TenantId_Admin")));
        ...
        //------
        [HttpPost]
        public async Task<IActionResult> UpdateDetails_Admin([FromBody] WellAI.Advisor.Model.Common.UpdateSubscriptionViewModel updateSubscriptionViewModel)
        {
