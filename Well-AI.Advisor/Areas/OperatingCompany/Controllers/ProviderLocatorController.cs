...
namespace WellAI.Advisor.Areas.OperatingCompany.Controllers
{
    ...
    public class ProviderLocatorController : BaseController
    {
        ...
        public ProviderLocatorController(UserManager<WellIdentityUser> userManager,
        ...
        [Route("ProviderLocator/{driver}")]
        public IActionResult DriverLocation(string driver)
        {
            try
            {
                if (_signInManager.IsSignedIn(User) == false)
                ...
                byte[] userdata = Convert.FromBase64String(driver);
                string userId = System.Text.Encoding.UTF8.GetString(userdata);
            ...
            {
                CustomErrorHandler customErrorHandler = new CustomErrorHandler(_roleManager, _userManager, db, Guid.Parse(_userManager.GetUserId(User)), Guid.Parse(WellAIAppContext.Current.Session.GetString("TenantId")));
                customErrorHandler.WriteError(ex, "Provider Locator", User.Identity.Name);
                string returnUrl = @"/Dashboard/Error";
                return LocalRedirect(returnUrl);
            }
        }
