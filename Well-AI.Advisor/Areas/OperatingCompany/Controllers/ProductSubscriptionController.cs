...
namespace WellAI.Advisor.Areas.OperatingCompany.Controllers
{
    ...
    public class ProductSubscriptionController : BaseController
    {
        ...
        public IActionResult ChangepaymentmethodOP()
        {
            try
            {
                //if (_signInManager.IsSignedIn(User) == false)
                ...
                //   return View();
                return PartialView("ChangepaymentmethodOP");
            }
            catch (Exception ex)
            {
                CustomErrorHandler customErrorHandler = new CustomErrorHandler(_roleManager, _userManager, db, Guid.Parse(_userManager.GetUserId(User)), Guid.Parse(WellAIAppContext.Current.Session.GetString("TenantId")));
        ...
        public async Task<IActionResult> UpdateDetails([FromBody] WellAI.Advisor.Model.Common.UpdateSubscriptionViewModel updateSubscriptionViewModel)
        {
            ...
            else
            {
                ...
                if (crmUserBasicDetail != null)
                {
                   ...
                   ICommonBusiness commonBusiness1 = new CommonBusiness(db, _roleManager, _userManager);
