...
namespace WellAI.Advisor.Areas.OperatingCompany.Controllers
{
    ...
    public class DispatchController : BaseController
    {
        ...
        public DispatchController(UserManager<WellIdentityUser> userManager,
        ...
        public async Task<IActionResult> GetDispatchList_Providerlocator([DataSourceRequest] DataSourceRequest request, string userId)
        {
        ...
        public async Task<IActionResult> DispatchDataDestroy(string dispatchId, string userId)
        {
            try
            {
                if (!string.IsNullOrEmpty(dispatchId))
                {
                    ...
                    var result = db.DispatchRoutes.Where(x => x.DispatchId == dispatchId).AsNoTracking().FirstOrDefault();
                    if (result != null)
                    {
        ...
        [HttpGet]
        public async Task<JsonResult> GetOperatorsharedetails_Providerlocator(string userId)
        {
            bool userStatus = false;
        ...
        public async Task<IActionResult> GetDispatchList_v3([DataSourceRequest] DataSourceRequest request, string userId)
        {
            List<DispatchRoutesModel> result = new List<DispatchRoutesModel>();
            List<RigList> rigList = new List<RigList>();
            try
        ...
        [HttpGet]
        public async Task<JsonResult> GetUserDestinations_v3(string userId)
        {
            userdestinations destinations = new userdestinations();
            CommonBusiness objComBusiness = new CommonBusiness(db, _roleManager, _userManager);
            List<RigList> rigList = new List<RigList>();
