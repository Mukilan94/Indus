...
{
    [Area("ServiceCompany")]
    public class WellMetricsSRVController : BaseController
    {
        private readonly ILogger<WellMetricsSRVController> _logger;
        RoleManager<IdentityRole> _roleManager;
        UserManager<WellIdentityUser> _userManager;
        private readonly SignInManager<WellIdentityUser> _signInManager;
        private readonly WebAIAdvisorContext db;
        public WellMetricsSRVController(UserManager<WellIdentityUser> userManager,SignInManager<WellIdentityUser> signInManager,
                                        RoleManager<IdentityRole> roleManager,WebAIAdvisorContext dbContext, ILogger<WellMetricsSRVController> logger)
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                var wellboard = new WellMetricsSRVModel
                {
                    TotalWells = 280,
                    ActiveWells = 170,
                    InactiveWells = 80,
            ...
            catch (Exception ex)
            {
                ...
                customErrorHandler.WriteError(ex, "wellMetricsSRV Index", User.Identity.Name);
        ...
        private List<Well> GetWellData()
        {
            ...
            {
                CustomErrorHandler customErrorHandler = new CustomErrorHandler(_roleManager, _userManager, db, Guid.Parse(_userManager.GetUserId(User)), Guid.Parse(WellAIAppContext.Current.Session.GetString("TenantId")));
                customErrorHandler.WriteError(ex, "wellMetricsSRV GetWellData", User.Identity.Name);
                return null;
            }
        }
        public IActionResult WellMetricsProfile(string id)
        {
            Well wellprofile = new Well();
            try
            {
                var wells = GetWellData();
                wellprofile = wells.FirstOrDefault(x => x.Name == id);
                if (wellprofile == null)
                {
            ...
            {
                CustomErrorHandler customErrorHandler = new CustomErrorHandler(_roleManager, _userManager, db, Guid.Parse(_userManager.GetUserId(User)), Guid.Parse(WellAIAppContext.Current.Session.GetString("TenantId")));
                customErrorHandler.WriteError(ex, "wellMetricsSRV WellMetricsProfile", User.Identity.Name);
