...
namespace WellAI.Advisor.Areas.ServiceCompany.Controllers
{
    ...
    public class UserManagerController : BaseController
    {
        ...
        public UserManagerController(ILogger<UserManagerController> logger,UserManager<WellIdentityUser> userManager,
                                    SignInManager<WellIdentityUser> signInManager,RoleManager<IdentityRole> roleManager,WebAIAdvisorContext dbContext)
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                foreach (var user in result)
                {
                    ...
                    var resultRole = _userManager.GetRolesAsync(user);
                    userViewModelList.Add(userViewModel);
                }
                ViewData["users"] = userViewModelList;
            }
            catch (System.Exception ex)
            {
                CustomErrorHandler customErrorHandler = new CustomErrorHandler(_roleManager, _userManager, db, Guid.Parse(_userManager.GetUserId(User)), Guid.Parse(WellAIAppContext.Current.Session.GetString("TenantId")));
                customErrorHandler.WriteError(ex, "UserManagerSRV Index", User.Identity.Name);
